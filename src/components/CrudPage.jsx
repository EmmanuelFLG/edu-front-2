...
export default function CrudPage({
  ...
  formTitle = { create: 'Novo registro', edit: 'Editar registro' }, action, importCsv,
}) {
  ...
  async function salvar(e) {
    e.preventDefault()
    const erros = {}
    fields.forEach((f) => { if (f.required && !String(valores[f.name] ?? '').trim()) erros[f.name] = 'Obrigatório' })
    if (Object.keys(erros).length > 0) return setErrosCampos(erros)

    setSalvando(true)
    try {
      const payload = {}
      fields.forEach((f) => (payload[f.name] = f.parse ? f.parse(valores[f.name]) : valores[f.name]))
  ...
  return (
    <div>
      ...
      <Modal open={modalAberto} onClose={() => setModalAberto(false)} title={registroEditando ? formTitle.edit : formTitle.create} size="max-w-2xl">
        <form onSubmit={salvar} className="space-y-6">
          {Object.entries(fields.reduce((acc, f) => { (acc[f.section || 'Geral'] = acc[f.section || 'Geral'] || []).push(f); return acc }, {})).map(([secao, campos]) => (
