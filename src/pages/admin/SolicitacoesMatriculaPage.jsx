...
import { ClipboardCheck, Inbox, Check, X } from 'lucide-react'
...
export default function SolicitacoesMatriculaPage() {
  ...
  const carregar = useCallback(async () => {
    setCarregando(true)
    try {
      setData(await solicitacaoMatriculaApi.listar({ page, size: 10, sort: 'dataSolicitacao,desc', status: status || undefined }))
    } catch (err) {
    ...
    try {
      ...
      if (resultado.alunoId) {
        try {
          ...
          // segue com a mensagem simples se a busca do aluno falhar
        }
      }
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Pedidos enviados pela tela pública de auto-matrícula, aguardando análise da secretaria."
      />
      ...
      <div className="card overflow-hidden mt-4">
        <div className="flex gap-1.5 border-b border-ink-50 p-3 bg-white">
          <button
            onClick={() => mudarStatus('')}
            className={`rounded-md px-3 py-1.5 text-xs font-semibold ${!status ? 'bg-ink-700 text-white' : 'text-ink-500 hover:bg-ink-50'}`}
          >
            Todas
          </button>
          {STATUS_SOLICITACAO.map((s) => (
            <button
              ...
              className={`rounded-md px-3 py-1.5 text-xs font-semibold ${status === s.value ? 'bg-ink-700 text-white' : 'text-ink-500 hover:bg-ink-50'}`}
            >
              {s.label}
            </button>
          ))}
        ...
        {carregando ? (
          ...
          <EmptyState
            ...
            description="Assim que alguém preencher o formulário público de matrícula, o pedido aparece aqui."
          />
        ) : (
              ...
              <tbody>
                {data.content.map((row) => (
                  <tr key={row.id} className="border-b border-ink-50 last:border-0 hover:bg-ink-50/40 align-top">
                    ...
                    <td className="px-5 py-3">
                      <Badge tone={STATUS_TONE[row.status] || 'neutral'}>{STATUS_SOLICITACAO.find((s) => s.value === row.status)?.label || row.status}</Badge>
      ...
      <ConfirmDialog
        ...
        loading={processandoAprovacao}
        description={
          aprovando
            ? `${aprovando.nome} será matriculado(a) e o sistema vai atribuir automaticamente uma turma de "${aprovando.serie}" no turno ${TURNO_LABEL[aprovando.turno] || aprovando.turno} (a com menos alunos, entre as que tiverem vaga).`
            : ''
        }
      />
